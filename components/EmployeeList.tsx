  ...
  if (employees.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-12 text-center">
        <p className="text-slate-400 font-medium">Nenhum servidor encontrado.</p>
      </div>
    );
  }
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase">Servidor</th>
              ...
              <tr key={emp.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold ${
                    emp.shift === 'Integral' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'
                  }`}>
                    {emp.shift}
                  </span>
                </td>
